...
if size == 0 {
	...
	asize = size + 6; // account for boundary tags
	asize = (asize + DSIZE - 1) & ~(DSIZE - 1); // align to double word
	awords = max(MIN_EXPLICIT_BLOCK_SIZE_WORDS, asize / WSIZE);
	
}

// Then, search the list of blocks for a free block
var handle = ds_mm_explicit_find_fit(mm, awords);
...
if handle == undefined {
	ds_mm_explicit_extend(mm, CHUNKSIZE);
	...
	handle = ds_mm_explicit_find_fit(mm, awords);
}

// Then, place the block in the right place and possibly split
ds_mm_explicit_place(mm, handle, awords);

// Return a handle to the memory, not the tag
