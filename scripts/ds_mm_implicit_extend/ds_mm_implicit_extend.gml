...
// in doing so, we subsume the old epilogue pointer
var new_head = old_end - WSIZE;

ds_mm_implicit_mark_free(mm, new_head, words);
...
var next_block = ds_mm_next_block(mm, new_head);

buffer_seek(mm[0], buffer_seek_start, next_block);
buffer_write(mm[0], buffer_u32, FENCE);

// Finally (TODO), coalesce if the previous block was free